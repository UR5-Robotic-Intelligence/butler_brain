...
class ButlerBrain():
  ...
  def compare_commands(self, rob_commands, label_rob_commands, container, act_name, res):
    ...
    print(label_rob_commands_str == new_res)
    ...
    # bleu similarity
    # weights = [min(1, len(label_rob_commands_str.split('\n'))/len(new_res.split('\n'))), 1]
    # bleu_score = sentence_bleu([label_rob_commands_str.split('\n')], new_res.split('\n'))
    ...
    if len(rob_commands_list) != len(label_rob_commands_list):
      n_mistakes += 1 # missing/extra command
      cmds = rob_commands_list if len(rob_commands_list) > len(label_rob_commands_list) else label_rob_commands_list
      other_cmds = label_rob_commands_list if cmds == rob_commands_list else rob_commands_list
      for i, cmd in enumerate(cmds):
        if cmd not in other_cmds:
          n_mistakes += 1 # missing/wrong command
          print("Mistake: ", cmd, " not in ", other_cmds)
          mistakes['label'].append(cmd if cmds == label_rob_commands_list else '')
          mistakes['pred'].append(cmd if cmds == rob_commands_list else '')
    else:
      for i, (pred, label) in enumerate(zip(rob_commands_list, label_rob_commands_list)):
        if pred != label:
          n_mistakes += 1 # wrong command or order
          mistakes['label'].append(label)
          mistakes['pred'].append(pred)
    if n_mistakes > 0:
