...
class ButlerBrain():
  def __init__(self, parsed_args) -> None:
    ...
    self.use_previous_exp = parsed_args.use_previous_exp
    ...
    if self.load_activities or self.load_experiment_data:
      ...
      self.prev_activities = deepcopy(self.new_activities)
      ...
      if triple_file_name is not None:
        ...
        try:
          ...
          res = ['']
          print("WARNING::no triples found in: ", triple_file_name)
        if len(res) == 0:
  ...
  def gpt_string_commands_to_list(self, gpt_string, return_components=False, use_gpt_string=True,
    ...
    try:
      ...
      # container = "unknown"
      # gpt_string[-1] = "container(unknown)"
    if use_gpt_string:
    ...
    for i, step in enumerate(gpt_string):
      ...
      if not use_gpt_string:
        ...
        try:
          ...
          raise Exception("SYNTAX ERROR IN GPT STRING WITH ERROR: ", e, "STEP: ", step)
  ...
  def compare_commands(self, rob_commands, label_rob_commands, container, act_name, res):
    ...
    container_ = rob_commands_list[-1]
    rob_commands_list = [cmd for cmd in rob_commands_list[:-1] if 'container(' not in cmd]
    rob_commands_list.append(container_)
  ...
  def add_new_activity(self, data_point, predict=False, idx=None):
    ...
    additional_prompts = ""
    if self.use_experience or self.use_previous_exp:
      activities = self.new_activities if not self.use_previous_exp else self.prev_activities
    ...
    return self.new_activities[act_name]
